...
test("chunked hyperlink delivery then resize", () => {
  const handle = wasm.create(80, 24, 10_000_000);
  expect(handle).toBeGreaterThan(0);
...
test("rapid resize with hyperlinks", () => {
...
test("large session with mixed content then resize (botster scenario)", () => {
...
test("large session WITHOUT hyperlinks then resize", () => {
  const handle = wasm.create(175, 43, 10_000_000);
  expect(handle).toBeGreaterThan(0);
